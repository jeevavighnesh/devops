...
update_repository() {
	sudo apt-get update >> /dev/null || { echo "Error while updating repository"; return; }
}
...
remove_server() {
	...
	sudo apt-get purge $package_name -y >> /dev/null || { echo "Could not remove $package_name completely an error occured"; exit 1; }
	sudo apt-get remove $package_name -y >> /dev/null
